		...
		(function (factory) {
			...
			ko.bindingHandlers['attr'] = {
				'update': function (element, valueAccessor, allBindings) {
					...
					ko.utils.objectForEach(value, function (attrName, attrValue) {
						...
						var src;
						if (attrName == 'src') {
							if (ko.isObservable(attrValue)) {
								if (attrValue.src === undefined) {
									attrValue.src = element.getAttribute('src');
								}
								src = attrValue.src;
							} else {
						...
						// kimleng
						if (attrName == 'src') {
							if (src != null && src != '' && (attrValue == null || attrValue == '')) {
								attrValue = src;
							}
						}
						// kimleng
				...
				makeTemplateValueAccessor: function (valueAccessor) {
					return function () {
						...
						if ((!unwrappedValue) || typeof unwrappedValue.length == "number")
							return {
								'foreach': modelValue, 'templateEngine': ko.nativeTemplateEngine.instance
							};
			...
			(function () {
				...
				ko.bindingHandlers['textInput'] = {
					'init': function (element, valueAccessor, allBindings) {
						// kimleng
						if (ko.isObservable(valueAccessor())) valueAccessor().element = element;
						// kimleng
			...
			(function () {
				...
				function mapNodeAndRefreshWhenChanged(containerNode, mapping, valueToMap, callbackAfterAddingNodes, index) {
					...
					var dependentObservable = ko.dependentObservable(function () {
						...
						disposeWhenNodeIsRemoved: containerNode, disposeWhen: function () {
							return !ko.utils.anyDomNodeIsAttachedToDocument(mappedNodes);
						}
					});
					return {
						mappedNodes: mappedNodes, dependentObservable: (dependentObservable.isActive() ? dependentObservable : undefined)
					};
				}
						...
						switch (editScriptItem['status']) {
							...
							case "added":
								if (movedIndex !== undefined) {
									...
									mapData = {
										arrayEntry: editScriptItem['value'], indexObservable: ko.observable(newMappingResultIndex++)
									};
									newMappingResult.push(mapData);
									itemsToProcess.push(mapData);
									if (!isFirstExecution)
